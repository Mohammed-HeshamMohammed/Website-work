...
// Remove XLSX and import Papaparse and ExcelJS instead.
...
export function OrderForm() {
  ...
  // Modified file upload handler using Papaparse for CSV and ExcelJS for Excel files.
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (fileExtension === "csv") {
        ...
        // Use ExcelJS to parse Excel files
        ...
        reader.onload = async (e) => {
          try {
            ...
            // Get the total number of rows (adjust if you have a header row)
            const rowCount = worksheet.rowCount
            setRecordCount(rowCount)
            setFormData((prev) => ({ ...prev, quantity: rowCount }))
          } catch (error) {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/paypal/create-order", {
        ...
        body: JSON.stringify({
          ...
          quantity, // For manual entry or file row count
          orderType,
