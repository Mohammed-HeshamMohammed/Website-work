  ...
  return (
    <section className="py-16 md:py-32 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-32 md:h-64 bg-gradient-to-b from-blue-50 to-transparent" />
        <div className="absolute bottom-0 left-0 w-full h-32 md:h-64 bg-gradient-to-t from-blue-50 to-transparent" />
        {/* Only show animations on larger screens for performance */}
        <motion.div
          className="absolute top-1/4 left-1/4 w-64 md:w-96 h-64 md:h-96 rounded-full bg-blue-100 blur-3xl opacity-30 md:opacity-50"
          animate={{
            x: [0, 50, 0, -50, 0],
            y: [0, -50, 0, 50, 0],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/4 right-1/4 w-64 md:w-96 h-64 md:h-96 rounded-full bg-blue-100 blur-3xl opacity-30 md:opacity-50"
          animate={{
            x: [0, -50, 0, 50, 0],
            y: [0, 50, 0, -50, 0],
          }}
      ...
      <div className="container mx-auto px-4 relative z-10">
        {/* Section Header */}
        <motion.div 
          className="text-center mb-10 md:mb-16 max-w-3xl mx-auto px-2 md:px-0"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          viewport={{ once: true }}
        >
          <span className="inline-block px-3 md:px-4 py-1 md:py-2 bg-blue-50 text-blue-600 font-medium rounded-full text-xs md:text-sm mb-3 md:mb-4">OUR SERVICES</span>
          <h2 className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-blue-900 to-blue-500 bg-clip-text text-transparent mb-4 md:mb-6">
        ...
        {/* Services Grid - switch to 1 column on mobile, 2 on tablet, 3 on desktop */}
              ...
              <Card className="h-full border-0 bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 relative">
                ...
                <div className="absolute top-4 md:top-6 right-4 md:right-6">
                  <motion.div
                    ...
                    className="flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full text-blue-900 font-bold text-lg md:text-xl bg-blue-50 group-hover:bg-blue-100 transition-colors duration-300"
                  >
                    {service.number}
                  </motion.div>
                </div>
                ...
                {/* Content */}
                <div className="p-5 md:p-8 pt-12 md:pt-16">
                  <motion.h3 
                    className="text-xl md:text-2xl font-bold text-blue-900 mb-3 md:mb-4 group-hover:text-blue-600 transition-colors duration-300"
                    initial={false}
                    whileHover={{ x: 5 }}
                  >
                    {service.title}
                  ...
                  <div className="mt-auto">
                    <motion.div
                      initial={false}
                      whileHover={{ x: 5 }}
                      className="inline-flex items-center text-blue-600 font-medium group-hover:text-blue-800 transition-colors duration-300 text-sm md:text-base"
                    >
                      Learn more
                      <svg 
                        xmlns="http://www.w3.org/2000/svg" 
                        className="h-4 w-4 md:h-5 md:w-5 ml-1 transform group-hover:translate-x-1 transition-transform duration-300" 
                        viewBox="0 0 20 20" 
                        fill="currentColor"
                      >
