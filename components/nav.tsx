...
import { Menu, X, Home, BarChart2, User } from "lucide-react"
...
export function Nav({ logoHeight = 48 }: { logoHeight?: number }) {
  ...
  // Animation for the active indicator bubble for bottom nav
  ...
  // Icon animation variants for bottom nav
  const iconVariants = {
    initial: { y: 0 },
    active: { 
      y: -5,
  ...
  return (
    <>
          ...
          >
            <Link href="/" className="block relative z-20">
              <Image
                ...
                height={logoHeight}
                priority
                className={`w-auto h-${Math.min(Math.max(Math.floor(logoHeight/7), 6), 16)}`}
              />
            </Link>
          </motion.div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white shadow-lg rounded-t-3xl overflow-hidden">
        <div className="flex items-center justify-around py-2">
          {navItems.slice(0, 3).map((item) => {
            const isActive = pathname === item.path
            // Map navItems to appropriate icons
            let Icon;
            if (item.name === "Home") {
              Icon = Home;
            } else if (item.name === "Plans") {
              Icon = BarChart2;
            } else if (item.name === "FAQ" || item.name === "About Us") {
              Icon = User;
            } else {
              Icon = Home; // Default fallback
            }
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute -top-1 w-12 h-12 bg-[#00B4D8]/10 rounded-full z-0"
                  />
                )}
                ...
                >
                  <Icon 
                    size={20} 
                    className={`${isActive ? "text-[#03045E]" : "text-gray-400"}`} 
                  />
                </motion.div>
                ...
                <span 
                  className={`mt-1 text-xs font-medium ${
                    isActive ? "text-[#03045E]" : "text-gray-400"
                  }`}
                >
                  {item.name}
                </span>
